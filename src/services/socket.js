...
class SocketService {
  ...
  _handleConnection(socket) {
    ...
    socket.on("chat:start", async (data) => {
      try {
        const { mode, topic, roleplayOption, level } = data;
        const sessionId = socket.id;

        // Chat oturumunu başlat
        const initialMessage = await chatService.startSession(sessionId, {
          mode,
          topic,
          roleplayOption,
          level,
        ...
        const { audioContent, audioId } = await ttsService.textToSpeech(
          initialMessage
        );
    ...
    socket.on("chat:message", async (data) => {
      try {
        ...
        if (!this.activeSessions.has(sessionId)) {
          throw new Error(
            "No active session found. Please start a chat first."
          );
        }
        ...
        if (!firstSentenceMatch) {
          ...
          const remainingText = completeMessage
            .slice(firstSentence.length)
            .trim();

          // İlk cümleyi TTS'e gönder
          const { audioContent: firstAudioContent, audioId: firstAudioId } =
            await ttsService.textToSpeech(firstSentence);
          ...
          if (remainingText) {
            ...
            const {
              audioContent: remainingAudioContent,
              audioId: remainingAudioId,
            } = await ttsService.textToSpeech(remainingText);
